...
<div class="menu">
	<ul>	
		<li><a target="_self" href="${ctx}/index.jsp">首&nbsp;&nbsp;页</a></li>
		<li><a target="_self" href="${ctx}/webs/news.jsp">新闻快讯</a></li>
		<li><a target="_self" href="${ctx}/webs/search.jsp">站内搜索</a></li>	
		<li><a target="_self" href="${ctx}/html/bbs.html">论坛</a></li>
		<li><a target="_self" href="${ctx}/html/blog.html">博客</a></li>
...
<script language='javascript'>
	...
	function editMember(){
		...
		<s:else>
			window.location='${ctx}/webs/editReg.jsp?tmp='+(new Date()).getTime();
		</s:else>		
	}
	...
	function search(){
		...
		if (p2==''){
			...
			if (p1=='newmer' || p1=='prommer'){//搜索所有商品
				window.location='${ctx}/webs/searchMer.jsp?searchtype='+p1+'&keyword='+escape(escape(p2));
			}else if (p1=='news'){//搜索商城资讯
				window.location='${ctx}/webs/searchNews.jsp?keyword='+escape(escape(p2));
			...
			<div class="login_main">			 	
				<s:if test="#session.member==null">
					...
					<s:form action="/webs/loginMember.action" id="loginform" target="_self">
						...
						<input type="image" src="${ctx}/images/login.jpg" onclick="checkForm(); return false;"/>
						&nbsp;&nbsp;<a href="${ctx}/webs/reg.jsp" target="_self">注册新会员</a>
					</s:form>					
				</s:if>				
				<s:else>
					<!-- 已登录 -->
					...
					<a href="${ctx}/webs/logoutMember.action" target="_self"><span class="blueText">安全退出</span></a>
				</s:else>
			</div>
...
<!--搜索区-->
		<div class="sou">
			...
			<div class="sou_right">
				<input type="image" src="${ctx}/images/search_right.jpg" name="submit" onclick="search()"/>
			</div>					
		</div>						
	</div>
